...
| Code block highlighting | @blocknote/code-block + limited Shiki bundle | 0.46.2 |
| Diagram rendering | Mermaid source storage; local fork editor placeholder | 11.14.0 |
| Whiteboard rendering | tldraw source storage; local fork editor placeholder | 4.5.10 |
...
Tauri setup keeps launch-time filesystem and subprocess work off the window creation critical path. Legacy `~/Laputa` housekeeping and the initial persisted-vault MCP bridge sync run on named background threads, and the initial bridge spawn waits briefly before launching the Node process so the React shell and vault list get the first CPU slice. Large legacy vaults, stale active-vault paths, or slow process startup should not beachball the macOS app before React mounts. React still resyncs the bridge from `useVaultSwitcher` after the persisted selection loads, and no selected vault stops the bridge. The renderer also defers nonessential AI agent, MCP status, and updater checks through `scheduleStartupIntegrationCheck()` so first paint is not competing with integration probes. The HTML bootstrap also installs a Tauri-only one-shot watchdog: React reports readiness from an effect after the root commits, and if that readiness signal never arrives the WebView reloads once instead of leaving macOS users in an inert rendered shell.

Local fork builds also keep optional editor-adjacent graphs out of the initial path where possible. Common icons use deep imports and a curated registry, code highlighting imports a limited Shiki language set, common date fields use native date inputs, AI-chat Markdown uses a small local renderer, unused collaboration/crash-reporting paths are shimmed, and Mermaid/tldraw source blocks render as durable placeholders in the main editor. `scripts/build-vite-stable.mjs` is the production Vite entrypoint for this fork and constrains Rollup file parallelism while reporting per-module memory progress during large builds. Minification is disabled by default for packaging reliability and can be re-enabled with `TOLARIA_ENABLE_MINIFY=1`.
...
```
App startup (deferred integration check)
  → useUpdater.check()
...
- **JS:** `@sentry/react` + `posthog-js` initialized lazily by `useTelemetry` hook; the React root also wires `onCaughtError`, `onUncaughtError`, and `onRecoverableError` through `Sentry.reactErrorHandler()` so production React invariants include component stack context when crash reporting is enabled. Local fork builds may alias `@sentry/react` to a no-op shim when crash-reporting delivery is intentionally disabled.
...
- `useUpdater(releaseChannel)` schedules its automatic startup check through `scheduleStartupIntegrationCheck()`, then calls Rust commands instead of hard-coding one updater endpoint in the frontend
...
- Local fork builds report no official update and reject install attempts unless compiled with `TOLARIA_ENABLE_OFFICIAL_UPDATES=1`, so a fork install is not silently replaced by the official release feed.
